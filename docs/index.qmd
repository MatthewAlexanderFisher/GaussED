...
Let $\delta$ denote a (bounded) linear functional acting on $f$. Then
...
The pair $\big(f, \delta(f)\big)$ is jointly Gaussian with
...
* Mean functions:
  ...
  \mathbb{E}[f(x)] = m(x), 
  \qquad 
  \mathbb{E}[\delta(f)] = \delta(m) \;=\; \int L(x)\, m(x)\,\mathrm{d}x,
  $$
...
* Covariances:
  ...
  $$
  \mathrm{Cov}[f(x), \delta(f)] 
  = \int L(x')\, k(x, x')\, \mathrm{d}x',
  $$

  $$
  \mathrm{Cov}[\delta(f), \delta(f')] 
  = \iint L(x)\, k(x, x')\, L(x')^\top \,\mathrm{d}x\,\mathrm{d}x'.
  $$

### Conditioning rule

Now, conditioning $f$ on observing $\delta(f)$, the posterior is again a GP:
...
m_\delta(x) 
= m(x) 
+ \mathrm{Cov}[f(x), \delta(f)] \,\mathrm{Var}[\delta(f)]^{-1} \, \big(y - \delta(m)\big),
$$

$$
k_\delta(x,x') 
= k(x,x') 
- \mathrm{Cov}[f(x), \delta(f)] \,\mathrm{Var}[\delta(f)]^{-1} \,\mathrm{Cov}[\delta(f), f(x')].
$$

This is just the **standard Gaussian conditioning formula**, where the “observation” is the random variable $\delta(f)$.
