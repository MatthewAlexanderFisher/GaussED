...
class Laplace(Basis):
    ...
    def __init__(self, dim, a=None, b=math.pi, c=0, d=None, dims=None):
        ...
        bases = [self]  # multiple bases

        self.dim = dim  # dimension of the basis functions generated
        self.input_dim = dim  # dimension of the expected x input

        if dims is None:
            self.dims = [i for i in range(self.dim)]  # list of the dimensions the laplace basis takes as input
            self.full_dims = True
        else:
        ...
        self.laplace_list = []  # this keeps track of laplace that are built from this object
    ...
    def eval(self, x, m):
        ...
        if self.fully_integrated:  # TODO: include normalisation constant when integrated
            ...
            j_mat_other = (j_mat.T[list(set([i for i in range(self.dim)]) - set(self.integral_dims))]).T

            return self.integral_const(m, j_mat_int) * self.a(x, j_mat_other) * torch.prod(
                    self.basis_func(x_r, j_mat_other), dim=2).reshape(n, m ** self.dim)

        else:
            return self.a(x, j_mat) * torch.prod(self.norm * self.basis_func(x_r, j_mat), dim=2).reshape(n, m ** self.dim)

    def set_new_basis(self, a, b, c, diff_orders, integral_dims, basis_func=None):
        """
        Sets a new basis for the Laplace object
        :param a:
        :param b:
        :param c:
        :param d:
        ...
        diff_orders_integral, diff_orders_other = diff_orders[integral_dims], diff_orders[non_integral_dims]
        b_integral, b_other = b[integral_dims], b[non_integral_dims]
        c_integral, c_other = c[integral_dims], c[non_integral_dims]

        if basis_func is None and non_integral_dims:  # if not all dimensions have been integrated
            ...
            if torch.equal(diff_orders_other, torch.zeros(self.dim)):
            ...
            elif torch.equal(diff_orders_other, (torch.ones(self.dim) * diff_orders_other[0])):
                ...
                def basis_func(x, i):
                    return index_func(i) * torch.stack(
                        [f(b_other[k] * x.T[k] + c_other[k], i.T[k].unsqueeze(1)) for k, f in enumerate(
                            self.func_vector)]).T
        ...
        if integral_dims:  # if integral_dims is not the empty list (i.e. we have an integral_transform)
            ...
            if torch.equal(diff_orders_integral, (torch.ones(self.dim) * diff_orders_integral[0])):

                self.integrated_func = diff_basis_func_gen(diff_orders_integral[0])
                self.integrated_func_vector = [self.integrated_func for i in range(len(integral_dims))]

                def integrated_basis_func(x, i):
                    x_t = x.unsqueeze(1)
                    return integrated_index_func(i) * self.integrated_func(b_integral * x_t + c_integral, i)
                ...
                for i in range(len(integral_dims)):
                    new_integrated_func_vector.append(diff_basis_func_gen(diff_orders_integral[i]))

                self.integrated_func_vector = new_integrated_func_vector

                def integrated_basis_func(x, i):
                    return integrated_index_func(i) * torch.stack(
                        [f(b_integral[k] * x.T[k] + c_integral[k], i.T[k].unsqueeze(1)) for k, f in
                         enumerate(self.integrated_func_vector)]).T
    ...
    def affine(laplace, linear):
        ...
        new_laplace.set_new_basis(new_a, laplace.b, laplace.c, laplace.diff_orders, laplace.integral_dims, laplace.normalisation)
    ...
    def simple_affine(laplace, A=1, B=0):
        ...
        new_laplace = Laplace(laplace.dim)
        new_laplace.set_new_basis(new_a, laplace.b, laplace.c, laplace.diff_orders, laplace.integral_dims)
    ...
    @staticmethod
    def input_affine(laplace, b, c):  # TODO: we are not performing affine transform - just overwriting b and c

        new_laplace = Laplace(laplace.dim)
        new_laplace.set_new_basis(laplace.a, b, c, laplace.diff_orders, laplace.integral_dims)
    ...
    def input_warp(laplace, func):
        ...
        new_laplace.set_new_basis(new_a, laplace.b, laplace.c, laplace.diff_orders, laplace.integral_dims, new_basis_func)
        new_laplace.set_dims(laplace.dims)
    ...
    # def sum(*args):
    #     """
    #     Returns a Laplace basis object TODO: can't transform from sum currently. would have to make the args be part of new laplace object and perform transform on each i in args
    #     :param args:
    #     :return:
    #     """
    #     dim = args[0].dim
    ...
    #             out = out + args[j + 1].eval(x, i)
    #         return out
    #
    #     new_laplace = Laplace(dim)
    #     new_laplace.eval = t_eval
    #     return new_laplace
    ...
    def differentiate(laplace, diff_dims, orders):
        ...
        relative_diff_dims, relative_diff_orders = laplace.get_relative_dims(diff_dims, diff_orders=orders)
        for j in range(len(relative_diff_dims)):
            diff_orders[relative_diff_dims[j]] = diff_orders[relative_diff_dims[j]] + relative_diff_orders[j]

        if laplace.differentiable:
            new_laplace = Laplace(laplace.dim)
            new_laplace.set_new_basis(laplace.a, laplace.b, laplace.c, diff_orders, laplace.integral_dims)
            new_laplace.set_differentiable(laplace)  # this is unnecessary since laplace is automatically differentiable
            new_laplace.set_integrable(laplace)
            new_laplace.set_dims(laplace.dims)
        else:
    ...
    def integrate(laplace, integral_dims, limits, method=None):
        """
        Overrides basis_func with integrated basis
        :param laplace:
        :param integral_dims: list of dimensions that are to do be integrated
        :param limits: the domain of integral_transform (currently a torch.Tensor([[-0.1,0.2],[-1,2],[-1.1,1]])
        :param method: If basis function not integrable - provide integration method
        """
        ...
        integral_dims = laplace.integral_dims + integral_dims  # check for repeated integral_transform dims?
        integral_dims.sort()
        ...
        if laplace.integrable:
            ...
            def integral_const(m, i):
                return torch.prod(
                    new_laplace.integrated_basis_func(limits.T, i)[1] - new_laplace.integrated_basis_func(limits.T, i)[
                        0], dim=1)

            new_laplace.integral_const = integral_const

            new_laplace.set_new_basis(laplace.a, laplace.b, laplace.c, new_diff_orders, integral_dims)
            new_laplace.set_differentiable(laplace)
            new_laplace.set_integrable(laplace)
            new_laplace.set_dims(laplace.dims)
    ...
    def line_integral(laplace, r, method, dr=None):
        ...
        new_laplace = Laplace(laplace.dim)
        new_laplace.set_new_basis(laplace.a, laplace.b, laplace.c, laplace.diff_orders, laplace.integral_dims,
                                  laplace.basis_func)
