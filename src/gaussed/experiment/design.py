...
class Design(Distribution):
    ...
    def update_basis_matrix(self, basis_matrix, design_point, m):
        """Update basis_matrix with a new basis_matrix evaluated at design_point

        Args:
            basis_matrix ([torch.Tensor]): [Matrix of basis function evaluations]
            design_point ([torch.Tensor]): [Design point to evaluate the new basis matrix to append]
            m ([int, list]): [Number of basis functions to use]

        Returns:
            [torch.Tensor]: [Updated basis matrix]
        """
        return torch.cat(
            [basis_matrix, self.basis_matrix(design_point.unsqueeze(0), m)]
        )
    ...
    def set_sample(self, sampling_func):
        ...
        Args:
            sampling_func ([function]): [Sampling function]
        """
    ...
    def set_sample_domain(self, sample_domain):
        """Sets the sample domain function

        Args:
            sample_domain ([function]): [description]
        """
        self.sample_domain = sample_domain
...
class EvaluationDesign(Design):
    ...
    def basis_matrix(self, design, m):
        ...
        for i in self.observables:
            basis_mats.append(
                i.basis_matrix(self.transform(design), m)
            )  # used to return torch.cat(basis_mats)

        out = torch.stack(basis_mats, dim=1)  # this interleaves the basis matrices
        return out.flatten(0, 1)

    def sample(
        self, d, mean, cov, n, random_sample=None, solver=None, sqrt=None, m=None
    ):
        x = self.transform(d)
        samples = []
        for i in self.observables:
    ...
    def inverse_transform(self, x):
        return torch.log(
            (x - self.d_lower) / (self.d_upper - self.d_lower)
