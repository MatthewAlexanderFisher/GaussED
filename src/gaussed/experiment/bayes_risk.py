...
class BayesRisk(Acquisition):
    ...
    def eval(self, d, phi_mat, y, m, n1=10, n2=10):
        ...
        mean, cov = self.qoi.condition(phi_mat, y, solver=self.solver, m=m)

        sn_samples = self.sn.sample(mean.size() + torch.Size([n1])).squeeze()
        new_ys = self.design.sample(d, mean, cov, n1, random_sample=sn_samples, solver=self.solver, m=m)

        # get covariance matrix at new design
        cov_new, Kyy_inv, Kcy = self.qoi.get_cov_matrices(phi_mat_new, nugget=self.nugget, solver=self.solver, m=m)
        sqrt_cov_new = self.solver.square_root(cov_new)

        if not self.solver.vectorisable:
            total_loss = 0
            for i in range(n1):
                qoi_samp = self.qoi.sample(mean, cov, n1, sn_samples.T[i].T.unsqueeze(1), solver=self.solver, m=m)
                y_new_i = torch.cat([y, new_ys[i]])
                mean_new = self.qoi.update_mean_vec(y_new_i, Kcy, Kyy_inv, solver=self.solver)

                qoi_new_samp = self.qoi.sample(mean_new, None, n2, sqrt=sqrt_cov_new, m=m)

                total_loss = total_loss + torch.sum(self.loss(qoi_samp, qoi_new_samp))

            total_loss = 1 / (n1 * n2) * total_loss
        else:
            qoi_samps = self.qoi.sample(mean, cov, n1, sn_samples, solver=self.solver, m=m)
            ...
            qoi_new_samps = self.qoi.sample(mean_new, None, n2, sqrt=sqrt_cov_new, m=m)
