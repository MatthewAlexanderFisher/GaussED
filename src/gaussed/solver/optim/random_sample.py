...
class SampleFirstOptimiser(Optimiser):
    ...
    def __init__(
        ...
        super().__init__(
            objective, optim_method, None, objective_params, optimiser_params
        )
    ...
    def run(self, n, *args, retain_graph=False, debug=False):
        ...
        if self.vectorisable_func is True:
            best_sample = sample_N[
                torch.argmin(self.objective(sample_N, **self.objective_params))
            ]
        else:
        ...
        for i in range(n):
            ...
            objective_eval = self.objective(
                self.variables[0], *args, **self.objective_params
            )
