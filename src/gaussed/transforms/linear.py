...
class Linear(Transform):
    ...
    def append_ids(self, dist):
        """Append distribution id to the list of all distribution ids

        Args:
            dist ([Distribution]): [Distribution object]
        """
        for i in range(len(hgp.SpectralGP.t_ids)):
            if id(self._distribution) in hgp.SpectralGP.t_ids[i]:
                hgp.SpectralGP.t_ids[i].append(id(dist))
                return

    def set_kernel_bases(self, new_bases):
        """Set the basis objects of the kernel.

        Args:
            new_bases ([list]): [List of basis objects]
        """
...
class Identity(Linear):
    ...
    def __call__(self):
        t_GP = hgp.SpectralGP(
            self.kernel, solver=self._distribution.solver, transform=True
        )
...
class Differentiate(Linear):
    ...
    def __call__(self):
        ...
        for i in range(len(self.kernel.bases)):
            t_bases.append(
                self.kernel.bases[i].differentiate(
                    self.kernel.bases[i], self.diff_dims, self.orders
                )
            )
...
class Integrate(Linear):
    ...
    def __call__(self):
        ...
        for i in range(len(self.kernel.bases)):
            t_bases.append(
                self.kernel.bases[i].integrate(
                    self.kernel.bases[i], self.dims, self.domain
                )
            )
        self.set_kernel_bases(t_bases)
...
class Affine(Linear):
    ...
    def __call__(self):
        if self._func_valued_transform is True:
            ...
            for i in range(len(self.kernel.bases)):
                t_bases.append(
                    self.kernel.bases[i].affine(self.kernel.bases[i], self.linear)
                )
        else:
            t_bases = []
            for i in range(len(self.kernel.bases)):
                t_bases.append(
                    self.kernel.bases[i].simple_affine(
                        self.kernel.bases[i], self.linear
                    )
                )
        ...
        t_GP.domain = self._distribution.domain
        t_GP.set_kernel_parameters(
            self._distribution.kernel.parameters
        )  # might want to check if linear has parameters
        self.append_ids(t_GP)
...
class Sum(Linear):
    ...
    def __call__(self):
        ...
        t_GP = hgp.SpectralGP(
            self.kernel, solver=self._distribution[0].solver, transform=True
        )
...
class SpatialDeformation(Linear):
    def __init__(self, distribution, func):
        super().__init__(distribution)
        self.kernel_copy = deepcopy(self.kernel)
        self.func = func
    ...
    def __call__(self):
        ...
        for i in range(len(self.kernel.bases)):
            t_bases.append(
                self.kernel.bases[i].input_warp(self.kernel.bases[i], self.func)
            )
        self.set_kernel_bases(t_bases)
...
class LineIntegral(Linear):
    ...
    def __call__(self):
        ...
        for i in range(len(self.kernel.bases)):
            t_bases.append(
                self.kernel.bases[i].line_integral(
                    self.kernel.bases[i], self.r, self.method, self.d_r
                )
            )
        self.set_kernel_bases(t_bases)
        ...
        t_GP = hgp.SpectralGP(
            self.kernel,
            solver=self._distribution.solver,
            transform=True,
            add_id=self.add_id,
        )
        t_GP.domain = self._distribution.domain
